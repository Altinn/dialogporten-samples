...
namespace CreateDialogSample;

public class CreateDialogDtoSamples
{

    /// <summary>
    /// A dialog with only the fields the API requires: the service resource it belongs to,
    /// the party it concerns, and a localized title. Everything else is left to the defaults
    /// set by Dialogporten, eg. the id, timestamps and <c>New</c> status.
    /// </summary>
    /// <returns>A <see cref="CreateDialog"/> ready to be posted as is.</returns>
    public static CreateDialog MinimalDialogDto()
    {
    ...
    /// <summary>
    /// A dialog exercising most of the create API,
    /// multilingual content with non-sensitive variants and a front channel embed, dialog
    /// and transmission level attachments, a request/submission transmission pair, GUI and
    /// API actions, an activity history and service owner only metadata.
    /// </summary>
    /// <returns>A fully populated <see cref="CreateDialog"/> useful as a reference for which fields exist and how they relate.</returns>
    public static CreateDialog ComplexDialogDto()
    {
        ...
        var requestTransmissionId = Guid.CreateVersion7();
        var submissionTransmissionId = Guid.CreateVersion7();

        return new CreateDialog
        {
            Id = Guid.CreateVersion7(),
            IdempotentKey = "byggesak-2026-004",
            ServiceResource = "urn:altinn:resource:super-simple-service",
            Party = "urn:altinn:person:identifier-no:08895699684",
